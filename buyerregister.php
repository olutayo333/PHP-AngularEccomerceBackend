<?php
    ...
    $password= $input['password'];
     ...
     $query= "SELECT * FROM  buyer_tb WHERE email= '$email' ";
   ...
   {        
        if($dbcon->num_rows>0){
            ...
            $query=" INSERT INTO buyer_tb (`name`, `phonenumber`, `email`, `password`) VALUES (?,?,?,?)" ;
            $prepare =$connect->prepare($query);
            $prepare->bind_param('ssss', $name, $phonenumber, $email, $hashp);
